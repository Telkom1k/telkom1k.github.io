...
install_base_system() {
    ...
    pacstrap -K /mnt \
        ...
        reflector \
        sbctl \
        systemd-ukify # Importante para UKI
...
log_chroot "Configurando mkinitcpio para UKI..."
# Asegurar que la hook 'uki' está presente para generar UKI
# Y que el preset del kernel hardened genera UKI
cat > /etc/mkinitcpio.conf << 'EOF_MKINITCPIO'
MODULES=()
BINARIES=()
FILES=()
HOOKS=(systemd autodetect microcode modconf kms keyboard sd-vconsole block sd-encrypt lvm2 filesystems fsck uki)
COMPRESSION="zstd"
EOF_MKINITCPIO

# Asegurar que el preset genera UKI (si no está ya por defecto con systemd-ukify)
# Asegúrate de que systemd-ukify está instalado (se incluye en install_base_system)
# Este comando editará el preset específico del kernel hardened
sed -i '/^#UKI_ENABLE="no"$/s/^#//' /etc/mkinitcpio.d/linux-hardened.preset
sed -i '/^UKI_ENABLE="no"$/s/"no"/"yes"/' /etc/mkinitcpio.d/linux-hardened.preset

# Regenerar UKI después de configurar cmdline y mkinitcpio
log_chroot "Generando Unified Kernel Image (UKI)..."
mkinitcpio -P

# Configurar AppArmor
log_chroot "Configurando AppArmor..."
...
# La UUID se inyectará desde el script principal después del chroot
...
# Habilitar enforce de AppArmor (lo tenías aquí, lo mantengo)
...
CHROOT_EOF
    ...
    # Actualizar cmdline con UUID real (AHORA SÍ, DESPUÉS DE QUE EL SCRIPT DE CHROOT LO HAYA CREADO)
    ...
    # sbctl se ejecuta sobre el /mnt porque sus operaciones son a nivel de archivos y no requieren chroot para generar o firmar
    # PERO el enroll-keys SÍ requiere que el sistema esté arrancado con sbctl o que se haga a mano en el BIOS
    # Vamos a manejar esto post-instalación para el usuario
    log "Configurando Secure Boot con sbctl..."
    arch-chroot /mnt sbctl create-keys || error "Fallo al crear claves Secure Boot."
    ...
    arch-chroot /mnt sbctl enroll-keys -m || warning "No se pudieron enrollar las claves en el firmware (Secure Boot no está en Setup Mode o está deshabilitado). Deberás hacerlo manualmente en el BIOS/UEFI después de reiniciar si quieres Secure Boot habilitado."

    # Firmar UKI con Secure Boot
    # El UKI se genera con mkinitcpio -P dentro del chroot. Su ruta será /boot/EFI/Linux/arch-hardened.efi
    # y /boot/EFI/BOOT/BOOTX64.EFI si systemd-boot está configurado para ello.
    ...
    if arch-chroot /mnt test -f "$UKI_PATH_IN_CHROOT"; then
        arch-chroot /mnt sbctl sign "$UKI_PATH_IN_CHROOT" || warning "Fallo al firmar el UKI."
    else
        warning "UKI ($UKI_PATH_IN_CHROOT) no encontrado para firmar."
    fi

    # Firmar el cargador de systemd-boot
    # Este es el cargador que el firmware ejecuta
    BOOTX64_PATH_IN_CHROOT="/boot/EFI/BOOT/BOOTX64.EFI"
    SYSTEMD_BOOTX64_PATH_IN_CHROOT="/boot/EFI/systemd/systemd-bootx64.efi" # El original
    if arch-chroot /mnt test -f "$BOOTX64_PATH_IN_CHROOT"; then
        arch-chroot /mnt sbctl sign "$BOOTX64_PATH_IN_CHROOT" || warning "Fallo al firmar BOOTX64.EFI."
    elif arch-chroot /mnt test -f "$SYSTEMD_BOOTX64_PATH_IN_CHROOT"; then
        # En algunos casos, BOOTX64.EFI es un symlink o una copia de este.
        arch-chroot /mnt sbctl sign "$SYSTEMD_BOOTX64_PATH_IN_CHROOT" || warning "Fallo al firmar systemd-bootx64.efi."
    else
        warning "Cargador de arranque EFI (BOOTX64.EFI o systemd-bootx64.efi) no encontrado para firmar."
    fi
...
EOF_APPARMOR
    ...
    # Aplicar perfiles
    arch-chroot /mnt aa-enforce /etc/apparmor.d/firefox || warning "Fallo al aplicar perfil AppArmor para Firefox. Verifica AppArmor y sus logs."
...
cleanup_system() {
    ...
    cat > /mnt/etc/systemd/system/cleanup.to.chroot.service << 'EOF_CLEANUP_SERVICE'
[Unit]
...
EOF_CLEANUP_SERVICE
    ...
    cat > /mnt/etc/systemd/system/cleanup.to.chroot.timer << 'EOF_CLEANUP_TIMER'
[Unit]
Description=Run system cleanup weekly
Requires=cleanup.to.chroot.service
...
EOF_CLEANUP_TIMER
    ...
    arch-chroot /mnt mv /etc/systemd/system/cleanup.to.chroot.service /etc/systemd/system/cleanup.service
    arch-chroot /mnt mv /etc/systemd/system/cleanup.to.chroot.timer /etc/systemd/system/cleanup.timer
    arch-chroot /mnt systemctl enable cleanup.timer

    # Limpiar archivos temporales en el entorno live
    log "Limpiando archivos temporales del entorno de instalación..."
...
main() {
    ...
    setup_apparmor # Ejecuta aa-enforce después de que configure_system habilita AppArmor
