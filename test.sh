...
cat > /etc/systemd/system/systemd-cryptsetup@cryptlvm.service.d/tpm2.conf << 'EOF_TPM2'
[Service]
ExecStart=
...
CHROOT_EOF
    ...
    if [ -n "$UKI_PATH" ]; then
        arch-chroot /mnt sbctl sign "${UKI_PATH#/boot}" # Changed /mnt to /boot here based on find output
    fi
