...
# Script de instalación de Arch Linux Hardened con UKI + NO TPM AUTODESCIFRADO
# Configuración ultra segura con cifrado completo, AppArmor y logging avanzado
# La contraseña LUKS se solicitará en cada reinicio.
...
install_base_system() {
    ...
    # Lista completa de paquetes necesarios (sin tpm2-tools ya que no se usará para descifrado)
    pacstrap -K /mnt \
        base base-devel \
        ...
        systemd-ukify \
        mkinitcpio \
        rsyslog fail2ban aide clamav # Paquetes para servicios de seguridad
...
setup_apparmor() {
    ...
    # Profile para navegador (Firefox)
...
# Este script se ejecuta dentro del entorno chroot para configurar el sistema instalado.
...
error_chroot() {
    echo -e "${RED}[CHROOT][ERROR] $1${NC}"
    exit 1
}

# Configurar zona horaria
log_chroot "Configurando zona horaria..."
ln -sf /usr/share/zoneinfo/Europe/Madrid /etc/localtime # Ajusta esto a tu zona
hwclock --systohc
...
echo "LANG=es_ES.UTF-8" > /etc/locale.conf # O en_US.UTF-8 si lo prefieres como LANG principal
echo "KEYMAP=es" > /etc/vconsole.conf # Muy importante para el teclado en el prompt de LUKS
...
mkdir -p /etc/kernel
# NOTA: CRYPTUUID será reemplazado por la UUID real DESPUÉS de que este script se ejecute en el chroot
...
BINARIES=""
FILES=""
HOOKS=(base systemd autodetect keyboard sd-vconsole modconf block sd-encrypt lvm2 filesystems fsck) # 'sd-encrypt' es crucial para pedir la contraseña
COMPRESSION="zstd"
EOF_MKINITCPIO

# Generar initramfs tradicional para el kernel hardened.
log_chroot "Generando initramfs para kernel hardened con mkinitcpio..."
mkinitcpio -P || error_chroot "Fallo al generar initramfs con mkinitcpio."
...
if [ -z "$KERNEL_VERSION" ]; then
    error_chroot "No se pudo determinar la versión del kernel linux-hardened instalado para UKI."
fi
...
bootctl install || error_chroot "Fallo al instalar systemd-boot."
systemctl enable systemd-boot-update || warning_chroot "No se pudo habilitar systemd-boot-update."
...
systemctl enable apparmor || warning_chroot "No se pudo habilitar AppArmor."
...
systemctl enable auditd || warning_chroot "No se pudo habilitar auditd."
...
systemctl enable systemd-journald || warning_chroot "No se pudo habilitar systemd-journald."

# Configurar rsyslog para logging adicional
log_chroot "Configurando rsyslog..."
mkdir -p /etc/rsyslog.d/ # Asegurar que el directorio existe
...
systemctl enable rsyslog || warning_chroot "No se pudo habilitar rsyslog."
...
systemctl enable NetworkManager || warning_chroot "No se pudo habilitar NetworkManager."
systemctl enable systemd-timesyncd || warning_chroot "No se pudo habilitar systemd-timesyncd."
systemctl enable fstrim.timer || warning_chroot "No se pudo habilitar fstrim.timer."
...
# ELIMINAR CUALQUIER CONFIGURACIÓN DE TPM PARA CRYPTSETUP
# Esto es crucial para que no intente el desbloqueo automático
log_chroot "Eliminando configuración de TPM para desbloqueo automático de LUKS..."
rm -f /etc/systemd/system/systemd-cryptsetup@cryptlvm.service.d/tpm2.conf
rm -f /etc/systemd/system/systemd-cryptsetup@dev-disk-by-uuid-*.service.d/tpm2.conf
systemctl daemon-reload || warning_chroot "Fallo al recargar la configuración de systemd."
...
CHROOT_EOF
    ...
    arch-chroot /mnt /configure_chroot.sh || error "Fallo durante la ejecución del script de configuración en chroot."

    # Obtener UUID de la partición cifrada y actualizar configuraciones FUERA del chroot
    # Esta variable se usa en el cmdline dentro del chroot
    CRYPT_UUID=$(blkid -s UUID -o value "$ROOT_PART")
    log "UUID de partición cifrada: $CRYPT_UUID"

    # Actualizar cmdline con UUID real
    arch-chroot /mnt sed -i "s/CRYPTUUID/$CRYPT_UUID/g" /etc/kernel/cmdline || error "Fallo al actualizar UUID en /etc/kernel/cmdline."
    
    # NO es necesario tocar tpm2.conf aquí porque lo eliminamos en el chroot script

    # Configurar contraseñas (ejecutado por el script principal fuera del chroot, pero afecta al chroot)
    log "Estableciendo contraseñas de root y usuario..."
    echo "root:$ROOT_PASSWORD" | arch-chroot /mnt chpasswd
    arch-chroot /mnt useradd -m -G wheel,audio,video,storage -s /bin/bash "$USERNAME" || warning "El usuario $USERNAME ya existe o hubo un problema al crearlo."
    ...
    log "Las claves de Secure Boot se han generado. Deberás enrollarlas manualmente en el firmware UEFI/BIOS después de reiniciar."
    warning "No se intentará el enrollado automático de claves Secure Boot."
...
# La función setup_tpm se elimina o comenta completamente
# setup_tpm() { ... }
...
main() {
    ...
    install_base_system
    generate_fstab
    setup_apparmor # Los perfiles se crean aquí, antes del chroot
    
    # --- configure_system() - Inicio de la sección CHROOT y su configuración ---
    log "Configurando sistema en chroot..."
...
CHROOT_EOF
    ...
    # === Fin de la sección configure_system() ===

    # La función setup_tpm se ha eliminado del script principal y del flujo de ejecución
    # setup_tpm # <-- Esta llamada se elimina
    ...
    echo "✓ Cifrado completo de disco con LUKS2 (contraseña manual requerida en cada inicio)"
    echo "✓ Kernel Linux Hardened"
    echo "✓ Unified Kernel Image (UKI)"
    echo "✗ TPM 2.0 para desbloqueo automático (deshabilitado por diseño)"
